...
void main() {
  test("Can encode a message", () {
    final message =
        PhoenixMessage("join_ref", "ref", "topic", "event", {"payload": ""});
    final json = message.toJSON();
    expect(json, '["join_ref","ref","topic","event",{"payload":""}]');
  });

  test("Can decode a payload into a message", () {
    const json = '["join_ref","ref","topic","event",{"payload":""}]';
  ...
  test("Can build a heartbeat message", () {
    final heartbeatMsg = PhoenixMessage.heartbeat("pendingHeartbeatRef");

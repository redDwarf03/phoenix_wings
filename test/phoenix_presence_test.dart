...
void main() {
  ...
  group("Presence construciton", () {
    test("Returns presence with defaults", () {
      ...
      expect(phoenixPresence.events.state,
          equals(PhoenixPresenceEvents.presenceState));
      expect(phoenixPresence.events.diff,
          equals(PhoenixPresenceEvents.presenceDiff));
    });
  });
  ...
  group("syncState", () {
    ...
    test("onJoins new presences and onLeave's left presences", () {
      ...
      final stateBefore = Map.from(state);
  ...
  group("syncDiff", () {
    test("syncs empty state", () {
      ...
      PhoenixPresence.syncDiff(
          state, {'joins': joins, 'leaves': {}}, null, null);
      expect(state, equals({}));

      final resultState = PhoenixPresence.syncDiff(
          state, {'joins': joins, 'leaves': {}}, null, null);
    ...
    test("removes presence when meta is empty and adds additional meta", () {
      ...
      final resultState = PhoenixPresence.syncDiff(
          state, {'joins': joins, 'leaves': leaves}, null, null);
  ...
  group("instance", () {
    ...
    test("syncs state and diffs", () {
      ...
      expect(
          phoenixPresence.list(by: listByFirst),
          equals([
            {'id': 1, 'phx_ref': '1'},
            {'id': 2, 'phx_ref': '2'}
          ]));

      _mockChannel!.triggerEvent("presence_diff", {
        'joins': {},
        'leaves': {'u1': user1}
      });
      expect(
    ...
    test("applies pending diff if state is not yet synced", () {
      ...
      phoenixPresence.onJoin((id, current, newPres) => {
            onJoins.add({'id': id, 'current': current, 'newPres': newPres})
          });
      phoenixPresence.onLeave((id, current, leftPres) => {
            onLeaves.add({'id': id, 'current': current, 'leftPres': leftPres})
          });
      ...
      _mockChannel!
          .triggerEvent("presence_diff", {'joins': {}, 'leaves': leaves});

      expect(phoenixPresence.list(by: listByFirst), equals([]));
      expect(phoenixPresence.pendingDiffs, [
        {'joins': {}, 'leaves': leaves}
      ]);
    ...
    test("allows custom channel events", () {
      final customEvents = PresenceEvents('the_state', 'the_diff');
      final phoenixPresence =
          PhoenixPresence(_mockChannel, opts: {'events': customEvents});
      ...
      _mockChannel!.triggerEvent("the_diff", {
        'joins': {},
        'leaves': {'user1': user1}
      });
      expect(phoenixPresence.list(by: listByFirst), equals([]));
